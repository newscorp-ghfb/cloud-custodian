...
def test_config_rule_policy():
    collection = PolicyLoader(Config.empty()).load_data(
        {'policies': [{'name': 'check-ec2', 'resource': 'ec2', 'mode': {'type': 'config-rule'}}]},
        file_uri=":mem:",
    )
    sam = {'Resources': {}}
    ...
    assert jmespath.search(
        'Resources.CheckEc2ConfigRule.Properties.Source.SourceIdentifier', sam
    ) == {'Fn::GetAtt': 'CheckEc2' + '.Arn'}
...
def test_periodic_policy():
            ...
            'policies': [
                {
                    'name': 'check-ec2',
                    'resource': 'ec2',
                    'mode': {'schedule': 'rate(1 hour)', 'type': 'periodic'},
                }
            ]
        },
        file_uri=":mem:",
    ...
    dispatch_render(p, sam)
    assert sam['Resources']['CheckEc2']['Properties']['Events'] == {
        'PolicySchedule': {'Type': 'Schedule', 'Properties': {'Schedule': 'rate(1 hour)'}}
    }
