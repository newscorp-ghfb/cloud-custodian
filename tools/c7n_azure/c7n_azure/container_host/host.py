...
class Host:
    def __init__(
        self, storage_id, queue_name, policy_uri, log_group=None, metrics=None, output_dir=None
    ):
    ...
    def _try_create_md5_content_hash(self, blob):
        ...
        try:
            ...
            client.create_blob_from_bytes(
                container, blob.name, blob_bytes.content, validate_content=True
            )
            ...
            log.warning(
                "Failed to apply a md5 content hash to policy {}. "
                    ...
                    for p in new_policies:
                        ...
                        if policy_mode == CONTAINER_TIME_TRIGGER_MODE:
                            ...
                            log.warning(
                                ...
                                "{} will not be run. "
                                "Supported policy modes include \"{}\" and \"{}\".".format(
                                    policy_mode,
                                    p.data['name'],
                                    CONTAINER_EVENT_TRIGGER_MODE,
                                    CONTAINER_TIME_TRIGGER_MODE,
    ...
    def poll_queue(self):
        ...
        if not self.queue_service:
            self.queue_service = Storage.get_queue_client_by_storage_account(
                self.queue_storage_account, self.storage_session
            )
            ...
            for message in messages:
                if message.dequeue_count > max_dequeue_count:
                    Storage.delete_queue_message(
                        self.queue_service, self.event_queue_name, message=message
                    )
    ...
    def prepare_queue_storage(self, queue_resource_id, queue_name):
        ...
        Storage.create_queue_from_storage_account(account, queue_name, self.session)
        return account
    ...
    def get_scheduler_config():
        ...
        return {
            ...
            'apscheduler.executors.default': {'class': executor, 'max_workers': '4'},
            'apscheduler.executors.threadpool': {'type': 'threadpool', 'max_workers': '20'},
    ...
    @staticmethod
    @click.command(
        help="Periodically run a set of policies from an Azure storage container "
    ...
    @click.option(
        "--storage-id",
        "-q",
        envvar=ENV_CONTAINER_STORAGE_RESOURCE_ID,
        required=True,
    ...
    @click.option(
        "--queue-name",
        "-n",
        envvar=ENV_CONTAINER_QUEUE_NAME,
        help="The name of the event queue to create",
    )
    @click.option(
        "--policy-uri",
        "-p",
        envvar=ENV_CONTAINER_POLICY_URI,
        required=True,
    ...
    @click.option(
        "--log-group",
        "-l",
        envvar=ENV_CONTAINER_OPTION_LOG_GROUP,
        help="Location to send policy logs",
    )
    @click.option(
        "--metrics",
        "-m",
        envvar=ENV_CONTAINER_OPTION_METRICS,
        help="The resource name or instrumentation key for uploading metrics",
    )
    @click.option(
        "--output-dir",
        "-d",
        envvar=ENV_CONTAINER_OPTION_OUTPUT_DIR,
