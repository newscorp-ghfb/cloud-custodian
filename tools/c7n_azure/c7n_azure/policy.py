...
class AzureModeCommon:
    ...
    def run_for_event(policy, event=None):
        ...
        with policy.ctx:
            ...
            policy.ctx.metrics.put_metric("ResourceTime", rt, "Seconds", Scope="Policy")
            policy._write_file('resources.json', utils.dumps(resources, indent=2))
            ...
            for action in policy.resource_manager.actions:
                ...
                try:
                    policy._write_file("action-%s" % action.name, utils.dumps(results))
