...
class CostFilterTest(BaseTest):
            ...
            self.load_policy(
                {
                    ...
                    'resource': 'azure.vm',
                    'filters': [
                        {'type': 'cost', 'timeframe': 'MonthToDate', 'op': 'eq', 'value': 1}
                    ],
                },
                validate=True,
            )
    ...
    def _verify_expected_call(self, mock, timeframe, resource_group):
        ...
        if isinstance(timeframe, int):
            ...
            self.assertEqual(
                definition.time_period.from_property, today - datetime.timedelta(days=timeframe)
            )
        else:
        ...
        if not resource_group:
            ...
            self.assertEqual(
                definition.dataset.filter.dimension.values, ['Microsoft.Compute/virtualMachines']
            )
