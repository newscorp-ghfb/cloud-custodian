...
class SessionTest(BaseTest):
    ...
    authorization_file_full = os.path.join(
        os.path.dirname(__file__), 'data', 'test_auth_file_full.json'
    )
    authorization_file_no_sub = os.path.join(
        os.path.dirname(__file__), 'data', 'test_auth_file_no_sub.json'
    )
    ...
    def test_initialize_session_auth_file_custom_subscription(self):
        s = Session(
            subscription_id=CUSTOM_SUBSCRIPTION_ID, authorization_file=self.authorization_file
        )
    ...
    def test_initialize_msi_auth_system(self):
        with patch.dict(
            os.environ,
            {constants.ENV_USE_MSI: 'true', constants.ENV_SUB_ID: DEFAULT_SUBSCRIPTION_ID},
            clear=True,
        ):
            s = Session()
    ...
    def test_initialize_session_token(self):
        with patch.dict(
            os.environ,
            {constants.ENV_ACCESS_TOKEN: 'token', constants.ENV_SUB_ID: DEFAULT_SUBSCRIPTION_ID},
            clear=True,
        ):
            s = Session()
    ...
    def xtest_get_function_target_subscription(self):
        with patch(
            ...
            with patch.dict(
                ...
                with patch(
                    'c7n_azure.session.ManagedGroupHelper.get_subscriptions_list', return_value=[]
                ):
                    s = Session()
    ...
    @patch(
        'c7n_azure.session.get_keyvault_secret',
