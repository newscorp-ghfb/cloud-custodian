...
class AzurePolicyModeTest(BaseTest):
            ...
            p = self.load_policy(
                {
                    ...
                    'mode': {
                        ...
                        'provision-options': {
                            ...
                            'storageAccount': {'name': 'testschemaname'},
                            'appInsights': {'name': 'testschemaname'},
                        },
                    },
                },
                validate=True,
            ...
            p = self.load_policy(
                {
                    ...
                    'mode': {
                        ...
                        'events': [
                            {
                                'resourceProvider': 'Microsoft.Network/networkSecurityGroups/securityRules',
                                'event': 'write',
                            }
                        ],
                    },
            ...
            p = self.load_policy(
                {
                    ...
                    'mode': {
                        'type': FUNCTION_EVENT_TRIGGER_MODE,
                        'events': ['KeyVaultWrite', 'ResourceGroupWrite', 'VmWrite'],
                    },
                },
                validate=True,
            )
    ...
    def test_azure_function_periodic_schema_schedule_valid(self):
        policy = {
            ...
            'resource': 'azure.vm',
            'mode': {'type': FUNCTION_TIME_TRIGGER_MODE, 'schedule': ''},
        }
    ...
    def test_container_periodic_schema_schedule_valid(self):
        policy = {
            ...
            'resource': 'azure.vm',
            'mode': {'type': CONTAINER_TIME_TRIGGER_MODE, 'schedule': ''},
        }

        valid_schedules = ['5 */2 * * fri', ' * 5 * feb * ', '5-7 * * * 1-5 ', '5,8,10 * * jan mon']
            ...
            p = self.load_policy(
                {
                    ...
                    'resource': 'azure.vm',
                    'mode': {'type': CONTAINER_EVENT_TRIGGER_MODE, 'events': ['VmWrite']},
                },
                validate=True,
            )
            self.assertTrue(p)
            ...
            p = self.load_policy(
                {
                    'name': 'test-azure-periodic-mode',
                    'resource': 'azure.vm',
                    'mode': {'type': CONTAINER_TIME_TRIGGER_MODE, 'schedule': '*/5 * * * *'},
                },
                validate=True,
            )
            self.assertTrue(p)
            ...
            self.load_policy(
                {
                    ...
                    'mode': {
                        'type': FUNCTION_EVENT_TRIGGER_MODE,
                        'events': ['VmWrite'],
                        'provision-options': {'identity': {'type': 'UserAssigned'}},
                    },
                },
                validate=True,
            )
        self.assertIn(
            'policy:something user assigned identity requires specifying id', str(em.exception)
        )
    ...
    def test_azure_function_unresolved_uai_identity(self):
        ...
        self.assertIn('policy:sm Could not find the user assigned identity mike', str(em.exception))
    ...
    def test_azure_function_resolved_uai_identity(self):
        ...
        exec_mode = p.get_execution_mode()
        uai = dict(name='mike', id='/subscriptions/xyz/userAssignedIdentities/foo', client_id='bob')
        session.client(
    ...
    def test_extract_properties(self):
        resource_id = (
            '/subscriptions/{0}/resourceGroups/rg/providers'
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    {'type': 'value', 'key': 'name', 'op': 'eq', 'value': 'test_emptyrg'},
                    {'type': 'empty-group'},
                ],
                'actions': [{'type': 'delete'}],
            }
    ...
    def test_empty_group_container_event(self, mock_delete):
        p = self.load_policy(
            {
                'name': 'test-azure-resource-group',
                'mode': {'type': CONTAINER_EVENT_TRIGGER_MODE, 'events': ['ResourceGroupWrite']},
                'resource': 'azure.resourcegroup',
                'filters': [
                    {'type': 'value', 'key': 'name', 'op': 'eq', 'value': 'test_emptyrg'},
                    {'type': 'empty-group'},
    ...
    def test_extract_resource_id(self):
        ...
        event = {'subject': rg_id}
        policy.resource_manager.resource_type.resource_type = 'resourceGroups'
    ...
    def get_sample_event():
        return {
            "subject": "/subscriptions/ea98974b-5d2a-4d98-a78a-382f3715d07e/"
