...
class ArmResourceTest(BaseTest):
    ...
    def test_arm_resource_schema_validate(self):
        with self.sign_out_patch():
            p = self.load_policy(
                {'name': 'test-azure-armresource', 'resource': 'azure.armresource'},
                validate=True,
            )
            self.assertTrue(p)
    ...
    def test_arm_resource_get_resources(self):
        rm = GenericArmResource(
            self.test_context,
            {'policies': [{'name': 'test', 'resource': 'azure.armresource'}]},
        )
        ...
        ids = [
            '{0}/providers/Microsoft.Compute/virtualMachines/cctestvm'.format(rg_id),
            rg_id,
        ]
