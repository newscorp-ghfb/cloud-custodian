...
class ArmResourceTest(BaseTest):
    ...
    def test_arm_resource_schema_validate(self):
        with self.sign_out_patch():
            p = self.load_policy(
                {'name': 'test-azure-armresource', 'resource': 'azure.armresource'}, validate=True
            )
            self.assertTrue(p)
    ...
    def test_metric_filter_invalid_missing_op(self):
        policy = {
            ...
            'filters': [
                ...
                {'type': 'metric', 'metric': 'Network In', 'aggregation': 'total', 'threshold': 0},
            ],
        }
        self.assertRaises(PolicyValidationError, self.load_policy, policy, validate=True)
    ...
    def test_metric_filter_invalid_missing_threshold(self):
        policy = {
            ...
            'filters': [
                ...
                {'type': 'metric', 'metric': 'Network In', 'aggregation': 'total', 'op': 'lt'},
            ],
        }
        self.assertRaises(PolicyValidationError, self.load_policy, policy, validate=True)
    ...
    @patch(
        'c7n_azure.resources.generic_arm_resource.GenericArmResourceQuery.filter',
    ...
    @patch('c7n_azure.query.ResourceQuery.filter', return_value=fake_arm_resources)
    @patch('c7n_azure.actions.delete.DeleteAction.process', return_value='')
    ...
    def test_arm_resource_get_resources(self):
        rm = GenericArmResource(
            self.test_context, {'policies': [{'name': 'test', 'resource': 'azure.armresource'}]}
        )

        rg_id = '/subscriptions/{0}/resourceGroups/test_vm'.format(
            self.session.get_subscription_id()
        )
        ids = ['{0}/providers/Microsoft.Compute/virtualMachines/cctestvm'.format(rg_id), rg_id]
        resources = rm.get_resources(ids)
        self.assertEqual(len(resources), 2)
        self.assertEqual(
            {r['type'] for r in resources}, {'resourceGroups', 'Microsoft.Compute/virtualMachines'}
        )
        self.assertEqual({r['id'] for r in resources}, set(ids))
        self.assertEqual({r['resourceGroup'] for r in resources}, {'test_vm'})
