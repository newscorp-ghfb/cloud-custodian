...
class StorageContainerTest(BaseTest):
    ...
    def test_storage_schema_validate(self):
        p = self.load_policy(
            {'name': 'test-storage-container', 'resource': 'azure.storage-container'}, validate=True
        )
        self.assertTrue(p)
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    {'type': 'value', 'key': 'name', 'op': 'glob', 'value': 'container*'},
                    {
                        'type': 'parent',
                        'filter': {
                            'type': 'value',
    ...
    def test_event(self):
        p = self.load_policy(
            {
                ...
                'mode': {'type': CONTAINER_EVENT_TRIGGER_MODE, 'events': ['StorageContainerWrite']},
                'resource': 'azure.storage-container',
            }
        )
        ...
        event = {
            "subject": "/subscriptions/" + DEFAULT_SUBSCRIPTION_ID + "/resourceGroups/"
