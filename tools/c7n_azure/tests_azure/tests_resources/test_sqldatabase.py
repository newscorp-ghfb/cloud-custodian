...
class SqlDatabaseTest(BaseTest):
    ...
    def test_sql_database_schema_validate(self):
        with self.sign_out_patch():
            p = self.load_policy(
                {'name': 'test-sql-database-schema-validate', 'resource': 'azure.sql-database'},
                validate=True,
            )
            self.assertTrue(p)
            ...
            p = self.load_policy(
                {'name': 'test-sql-database-schema-validate', 'resource': 'azure.sqldatabase'},
                validate=True,
            )
            self.assertTrue(p)
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    ...
                    {'type': 'value', 'key': 'sku.tier', 'op': 'eq', 'value': 'Standard'},
                ],
            }
        )
    ...
    def test_resize_action(self, update_mock):
        p = self.load_policy(
            {
                ...
                'resource': 'azure.sqldatabase',
                'filters': [{'type': 'value', 'key': 'name', 'value': 'cctestdb'}],
                'actions': [
                    {
                        'type': 'resize',
                        'tier': 'Standard',
        ...
        expected_db_update = DatabaseUpdate(
            sku=Sku(capacity=100, tier='Standard', name='Standard'), max_size_bytes=21474836480
        )
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    {'type': 'value', 'key': 'name', 'value': 'cctestdb'},
                    {'type': 'transparent-data-encryption', 'enabled': True},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    {'type': 'value', 'key': 'name', 'value': 'cctestdb'},
                    {'type': 'data-masking-policy', 'enabled': False},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'filters': [
                    {'type': 'value', 'key': 'name', 'op': 'eq', 'value': 'cctestdb'},
                    {'type': 'short-term-backup-retention-policy', 'retention-period-days': 14},
                ],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                'resource': 'azure.sqldatabase',
                'filters': [
                    {'type': 'value', 'key': 'name', 'op': 'eq', 'value': 'cclongtermretentiondb'},
                    {
                        'or': [
                            {
                                'type': 'long-term-backup-retention-policy',
...
class ShortTermBackupRetentionPolicyActionTest(BaseTest):
    ...
    def setUpClass(cls, *args, **kwargs):
        ...
        cls.client = (
            ...
            .client('azure.mgmt.sql.SqlManagementClient')
...
class LongTermBackupRetentionPolicyActionTest(BaseTest):
    ...
    def test_update_weekly_retention_policy(self):
        ...
        self._assert_retention_period_equal(
            BackupRetentionPolicyHelper.LongTermBackupType.weekly.retention_property, 'P10D'
        )
    ...
    def test_update_monthly_retention_policy(self):
        ...
        self._assert_retention_period_equal(
            BackupRetentionPolicyHelper.LongTermBackupType.monthly.retention_property, 'P6W'
        )
    ...
    def test_update_yearly_retention_policy(self):
        ...
        self._assert_retention_period_equal(
            BackupRetentionPolicyHelper.LongTermBackupType.yearly.retention_property, 'P2Y'
        )
