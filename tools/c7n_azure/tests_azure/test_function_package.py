...
class FunctionPackageTest(BaseTest):
        ...
        p = self.load_policy(
            {
                ...
                'mode': {
                    ...
                    'provision-options': {
                        'identity': {'id': 'awolf', 'client_id': 'dog', 'type': 'UserAssigned'}
                    },
        ...
        p = self.load_policy(
            {
                'name': 'test-azure-public-ip',
                'resource': 'azure.publicip',
                'mode': {'type': FUNCTION_EVENT_TRIGGER_MODE, 'events': ['PublicIpWrite']},
            }
        )
        ...
        p = self.load_policy(
            {
                ...
                'mode': {'type': FUNCTION_EVENT_TRIGGER_MODE, 'events': ['ResourceGroupWrite']},
            }
        )
    ...
    def test_publish(self, post_mock):
        ...
        packer.pkg = AzurePythonPackageArchive()
        creds = User(
            publishing_user_name='user', publishing_password='password', scm_uri='https://uri'
        )
        ...
        self.assertEqual(
            post_mock.call_args[0][0], 'https://uri/api/zipdeploy?isAsync=true&synctriggers=true'
        )
        self.assertEqual(
            post_mock.call_args[1]['headers']['content-type'], 'application/octet-stream'
        )
