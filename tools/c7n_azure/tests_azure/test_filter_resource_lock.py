        ...
        p = self.load_policy(
            {
                ...
                'filters': [{'type': 'resource-lock', 'lock-type': 'ReadOnly'}],
            },
            validate=True,
        )
        self.assertTrue(p)
        ...
        p = self.load_policy(
            {
                'name': 'test-lock-filter',
                'resource': 'azure.disk',
                'filters': [{'type': 'resource-lock', 'lock-type': 'CanNotDelete'}],
            }
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                'name': 'test-lock-filter',
                'resource': 'azure.disk',
                'filters': [{'type': 'resource-lock', 'lock-type': 'Any'}],
            }
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
        ...
        p = self.load_policy(
            {
                ...
                'filters': [{'type': 'resource-lock', 'lock-type': 'Absent'}],
            }
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
