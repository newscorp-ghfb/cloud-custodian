    ...
    with sqlite3.connect(db) as conn:
        ...
        if rows:
            ...
            try:
                ...
                log.error("Error inserting enis account:%s rows:%d", cfg['awsAccountId'], len(rows))
            stats['RowCount'] += len(rows)
...
@click.option('--end')
@click.option(
    '-r', '--resources', multiple=True, type=click.Choice(list(RESOURCE_FILE_INDEXERS.keys()))
)
    ...
    with executor(max_workers=multiprocessing.cpu_count()) as w:
        ...
        for a in account_ids:
            for r in resources:
                futures[
                    w.submit(
                        process_account_resources, a, bucket, prefix, region, store, start, end, r
                    )
                ] = (a, r)
