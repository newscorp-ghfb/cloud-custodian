    ...
    for p in paginator.paginate(**params):
        ...
        for e in p['events']:
            ...
            msg = get_sentry_message(
                settings,
                {'logEvents': [e], 'logStream': e['logStreamName'], 'logGroup': config.log_group},
                client,
            )
            if msg is None:
                continue
...
def send_sentry_message(sentry_dsn, msg):
    ...
    endpoint = "%s://%s/api/%s/store/" % (parsed.scheme, parsed.netloc.split('@')[1], project_id)
...
def get_sentry_message(config, data, log_client=None, is_lambda=True):
    ...
    if breadcrumbs:
        sentry_msg['breadcrumbs'] = [
            {'category': 'policy', 'message': e['message'], 'timestamp': e['timestamp'] / 1000}
...
def parse_traceback(msg, site_path="site-packages", in_app_prefix="c7n"):
    ...
    for l in lines[1:-1]:
        ...
        if l.startswith('Traceback'):
            ...
            err_ctx = dict(
                lineno=lineno, abs_path=abs_path, function=function, filename=f_path, module=module
            )
...
def orgreplay(options):
    ...
    def process_account(a):
        ...
        if team_name not in teams:
            ...
            spost(
                endpoint + "organizations/%s/teams/" % options.sentry_org, json={'name': team_name}
            )
            teams.add(team_name)
