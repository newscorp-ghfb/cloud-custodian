...
class SentrySenderTests(unittest.TestCase):
    ...
    def test_get_sentry_message(self):
        ...
        sentry_msg = c7nsentry.get_sentry_message(
            config,
            {
                'logGroup': '/cloud-custodian/',
                'logStream': 'night-policy',
                'logEvents': [{'message': msg, 'timestamp': time.time() * 1000}],
            },
        )
        self.assertEqual(
            sentry_msg['user'], {'id': config['account_id'], 'username': config['account_name']}
        )

    def test_preserve_full_message(self):
        emsg, error = c7nsentry.parse_traceback(msg2)
        self.assertIn("FinalDBSnapshotIdentifier is not a valid identifier", error['value'])
