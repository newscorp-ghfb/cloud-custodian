        ...
        for module_name in modules:
            ...
            if hasattr(module, '__path__'):
                ...
                if getattr(module, '__file__', None) is None:
                    ...
                    sitedir = os.path.abspath(
                        os.path.join(list(module.__path__)[0], os.pardir)
                    )
...
class LambdaManager:
    ...
    def _create_or_update(self, func, role=None, s3_uri=None, qualifier=None):
        ...
        if existing:
            ...
            if config_changed:
                log.debug(
                    "Updating function: %s config %s",
                    func.name,
                    ", ".join(sorted(config_changed)),
                )
...
class PolicyLambda(AbstractLambdaFunction):
    ...
    def get_events(self, session_factory):
        ...
        if self.policy.data['mode']['type'] in ('config-rule', 'config-poll-rule'):
            ...
            events.append(
                CloudWatchEventSource(self.policy.data['mode'], session_factory)
            )
        return events
...
class CloudWatchEventSource(AWSEventBase):
    ...
    def remove(self, func, func_deleted=True):
        if self.get(func.event_name):
            ...
            try:
                targets = self.client.list_targets_by_rule(Rule=func.event_name)[
                    'Targets'
                ]
                if targets:
                    self.client.remove_targets(
                        Rule=func.event_name, Ids=[t['Id'] for t in targets]
                    )
            except ClientError as e:
                log.warning(
                    "Could not remove targets for rule %s error: %s", func.name, e
                )
...
class SecurityHubAction:
    ...
    def get(self, name):
        ...
        arn = self._get_arn()
        actions = client.describe_action_targets(ActionTargetArns=[arn]).get(
            'ActionTargets', ()
        )
        ...
        params = {
            'Name': (
                ...
                or (
                    "%s %s"
                    % (
                        self.policy.resource_type.split('.')[-1].title(),
                        self.policy.name,
                    )
                )
            ),
        ...
        for queue_arn in self.queue_arns:
            ...
            if queue_arn in event_mappings:
                ...
                if (
                    ...
                    or mapping['BatchSize'] != self.batch_size
                ):
                    continue
                modified = True
            else:
                ...
                for page in paginator.paginate(TopicArn=topic_arn):
                    for subscription in page['Subscriptions']:
                        ...
                        try:
                            ...
                            log.debug(
                                "Unsubscribed %s from %s" % (func.name, topic_name)
                            )
...
class BucketSNSNotification(SNSSubscription):
    ...
    def get_topic(self, bucket):
        ...
        topic_arns = [
            t['TopicArn'] for t in all_topics if 's3:ObjectCreated:*' in t['Events']
        ]
        if not topic_arns:
