...
class CodeBuild(BaseTest):
    def test_config_source(self):
        ...
        resources = self.load_policy(
            {'name': 'dbuilders', 'resource': 'aws.codebuild'}, session_factory=factory
        ).run()
    ...
    def test_post_finding_build(self):
        ...
        shape_validate(
            rfinding['Details']['AwsCodeBuildProject'], 'AwsCodeBuildProjectDetails', 'securityhub'
        )
...
class CodePipeline(BaseTest):
    def test_config_pipeline(self):
        ...
        assert resource['artifactStore'] == {
            'type': 'S3',
            'location': 'mypipe-artifactbucketstore-4ebot00zlvbv',
        }
    ...
    def test_delete_pipeline(self):
        ...
        p = self.load_policy(
            {'name': 'del-pipe', 'resource': 'aws.codepipeline', 'actions': ['delete']},
            session_factory=factory,
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
        ...
        p = self.load_policy(
            {
                ...
                "filters": [{"type": "value", "key": "targetRevision.revisionType", "value": "S3"}],
                "actions": [{"type": "delete"}],
            },
            session_factory=factory,
        )
