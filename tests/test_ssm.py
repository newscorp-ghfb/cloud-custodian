...
class TestOpsCenter(BaseTest):
    def test_post_ops_item(self):
        ...
        p = self.load_policy(
            {
                ...
                'resource': 'aws.lambda',
                'source': 'config',
                'query': [{'clause': "resourceId = 'custodian-aws'"}],
                'actions': [{'type': 'post-item'}],
            },
        ...
        item = client.get_ops_item(OpsItemId=resources[0]['c7n:opsitem']).get('OpsItem')
        arn = p.resource_manager.get_arns(resources)[0]
        self.assertTrue(arn in item['OperationalData']['/aws/resources']['Value'])
    ...
    def test_ops_item_filter(self):
        ...
        self.assertEqual(resources[0]['c7n:opsitems'], ['oi-9be57440dcb3'])
        ...
        p = self.load_policy(
            {
                ...
                'resource': 'aws.lambda',
                'source': 'config',
                'query': [{'clause': "resourceId = 'custodian-nuke-emr'"}],
                'actions': [{'type': 'post-item'}],
            },
            session_factory=factory,
        )
    ...
    def test_update_ops_item(self):
        ...
        p = self.load_policy(
            {
                ...
                'query': [{'Key': 'Status', 'Operator': 'Equal', 'Values': ['Open']}],
                'actions': [
                    {
                        'type': 'update',
        ...
        if self.recording:
            time.sleep(5)
        item = client.get_ops_item(OpsItemId=resources[0]['OpsItemId'])['OpsItem']
        self.assertEqual(item['Status'], 'Resolved')
        self.assertEqual(
            item['Notifications'], [{'Arn': 'arn:aws:sns:us-west-2:644160558196:aws-command'}]
        )
    ...
    def test_invalid_resource_query(self):
        self.assertRaises(
            ...
            self.load_policy,
            {
                'name': 'value',
                'resource': 'aws.ops-item',
                'query': [{'Key': 'Status', 'Operator': 'Equals', 'Values': ['Open']}],
            },
            validate=True,
        )
    ...
    def test_get_resources(self):
        ...
        p = self.load_policy(
            {'name': 'foo', 'resource': 'aws.ops-item'},
            session_factory=factory,
            config={'region': 'us-east-1'},
        )
...
class TestSSM(BaseTest):
    def test_ec2_ssm_send_command_validate(self):
        self.assertRaises(
            ...
            self.load_policy,
            {
                ...
                'resource': 'aws.ec2',
                'actions': [
                    {'type': 'send-command', 'command': {'DocumentName': 'AWS-RunShellScript'}}
                ],
            },
            validate=True,
        )
    ...
    def test_ssm_send_command(self):
        ...
        result = (
            ...
            .get_command_invocation(
                InstanceId=resources[0]['InstanceId'], CommandId=resources[0]['c7n:SendCommand'][0]
            )
        )
    ...
    def test_ssm_parameter_delete(self):
        ...
        p = self.load_policy(
            {'name': 'ssm-param-tags', 'resource': 'ssm-parameter', 'actions': ['delete']},
    ...
    def test_ssm_parameter_delete_non_existant(self):
        ...
        p.resource_manager.actions[0].process([{'Name': 'unicorn'}])
    ...
    def test_ssm_parameter_tag_arn(self):
        ...
        self.assertEqual(
            resources[0]['Tags'],
            [{'Key': 'App', 'Value': 'GitterSearch'}, {'Key': 'Env', 'Value': 'Dev'}],
        )
    ...
    def test_ssm_parameter_not_secure(self):
        ...
        client.put_parameter(
            Name='secure-test-name', Type='SecureString', Overwrite=True, Value='secure-test-value'
        )
        ...
        p = self.load_policy(
            {
                ...
                "resource": "ssm-parameter",
                "filters": [{"type": "value", "op": "ne", "key": "Type", "value": "SecureString"}],
            },
            session_factory=session_factory,
        )
        resources = p.run()
        ...
        p = self.load_policy(
            {
                ...
                "filters": [{"type": "value", "key": "Expired", "value": True}],
            },
            session_factory=session_factory,
        )
        resources = p.run()
    ...
    def test_ssm_get_manager_instances(self):
        ...
        p = self.load_policy(
            {"name": "ssm-get-managed-instances", "resource": "ssm-managed-instance"},
            session_factory=session_factory,
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
        ...
        p = self.load_policy(
            {
                ...
                "filters": [{"type": "cross-account", "whitelist": ["xxxxxxxxxxxx"]}],
            },
            session_factory=session_factory,
        )
        ...
        p = self.load_policy(
            {
                ...
                "resource": "ssm-document",
                "filters": [{"type": "cross-account", "whitelist": ["xxxxxxxxxxxx"]}],
                "actions": [{"type": "set-sharing", "remove": "matched"}],
            },
            session_factory=session_factory,
        )
        p.run()
        ...
        p = self.load_policy(
            {
                ...
                "resource": "ssm-document",
                "actions": [{"type": "set-sharing", "add": ['yyyyyyyyyyyy']}],
            },
            session_factory=session_factory,
        )
        p.run()
        ...
        p = self.load_policy(
            {
                ...
                "resource": "ssm-document",
                "filters": [{"type": "cross-account", "whitelist": ["xxxxxxxxxxxx"]}],
                "actions": [{"type": "delete", "force": False}],
            },
            session_factory=session_factory,
        )
    ...
    def test_get_data_sync_resources(self):
        ...
        p = self.load_policy(
            {"name": "ssm-get-data-sync-resources", "resource": "ssm-data-sync"},
            session_factory=session_factory,
        )
        resources = p.run()
