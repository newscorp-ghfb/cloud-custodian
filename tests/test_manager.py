...
class TestEC2Manager(BaseTest):
    def get_manager(self, data, config=None, session_factory=None):
        ctx = ExecutionContext(
            session_factory, Bag({"name": "test-policy", 'provider_name': 'aws'}), config or {}
        )
        return EC2(ctx, data)
    ...
    def test_manager_iter_filters(self):
        p = self.load_policy(
            {
                ...
                'filters': [
                    {
                        'and': [
                            {'type': 'listener', 'key': 'Protocol', 'value': 'HTTP'},
                            {
                                'type': 'listener',
        ...
        self.assertEqual(
            [f.type for f in p.resource_manager.iter_filters()], ['and', 'listener', 'listener']
        )

    def test_trim_runtime_filters(self):
        filter_data = [
            {'and': [{'not': [{'type': 'event', 'key': 'xyz', 'value': 'bar'}]}, {'key': 'value'}]}
        ]
        ...
        p = self.load_policy(
            {
                'name': 'xyz',
                'resource': 'aws.ec2',
                'mode': {'type': 'config-rule', 'role': 'xyz'},
        ...
        self.assertEqual(
            [n is not None and n.type or n for n in m.iter_filters(block_end=True)],
            ['and', 'value', None],
        )
    ...
    def test_filters(self):
        ec2 = self.load_policy(
            {'name': 'xyz', 'resource': 'aws.ec2', 'filters': [{"tag:CMDBEnvironment": "absent"}]}
        ).resource_manager
        self.assertEqual(
    ...
    def test_actions(self):
        ...
        ec2 = self.load_policy(
            {'name': 'xyz', 'resource': 'aws.ec2', 'actions': ['mark']}
        ).resource_manager
        ...
        ec2 = self.load_policy(
            {
                'name': 'xyz',
                'resource': 'aws.ec2',
                "actions": [{"type": "mark", "value": "Missing proper tags"}],
            }
        ).resource_manager
