...
class TestQLDB(BaseTest):
    ...
    def test_qldb_force_delete(self):
        ...
        self.assertRaises(ClientError, client.describe_ledger, Name='devledger')

    def test_qldb_delete(self):
        factory = self.replay_flight_data('test_qldb_delete')
        output = self.capture_logging('custodian.actions')
        p = self.load_policy(
            {'name': 'qldb', 'resource': 'aws.qldb', 'actions': ['delete']}, session_factory=factory
        )
        resources = p.run()
        self.assertEqual(len(resources), 2)
        ...
        if self.recording:
            time.sleep(10)
        client = factory().client('qldb')
        self.assertRaises(ClientError, client.describe_ledger, Name='devx')
